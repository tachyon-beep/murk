...
Murk v0.1.6 is feature-complete against its original design. Everything
below is implemented, tested, and published — not planned, not stubbed.
...
| **Python** | PyO3/maturin bindings, Gymnasium `Env` + `VecEnv` adapters, 28 exposed types, PEP 561 type stubs | 72 passing Python tests including a full PPO training smoke test |
...
- **~30K LOC across 13 crates** with clean dependency layering
  (murk-core is a leaf; murk is a facade; everything composes)
- **660+ tests** — unit, integration, property-based (proptest), stress,
...
**Priority**: Critical — blocks training at scale.

Murk originated from the observation that the GIL is a bottleneck when
generating training data for PyTorch. The current `MurkVecEnv` creates
N independent Python-wrapped worlds. For self-play training with 50–100
agents, we need thousands of parallel environments with zero Python
involvement in the hot loop.

**Scope**:
- Rust-native `BatchedEngine` that manages N independent world
  instances in a single process
- Single Python call → step all N worlds → return batched observation
  tensors
...
**Impact**: Enables the self-play and league training regime that
produces the final 90% of Echelon's agent quality.
